...
# from e3doubt.radar_utils import get_receiver_az_el_geod
...
gdlat, glon, h = exp._points['gdlat'].values,exp._points['glon'].values, exp._points['h'].values
...
covmats = exp.velocity_cov_matrix()
...
xi_p, eta_p = grid.projection.geo2cube(exp._points['glon'].values, exp._points['gclat'].values)
...
v_e = np.zeros(exp.N['pt'])
v_n = np.zeros(exp.N['pt'])

v_tot = np.vstack((v_e.flatten(), v_n.flatten()))

v_coords = np.vstack((exp._points['glon'].values, exp._points['gclat'].values)) # E-field is defined on the grid
...
csaxes[0].scatter(exp._points['glon'].values, exp._points['gclat'].values,
                  marker='^', s=10,
                  zorder=21,
                  color='C2',
                  label='obs')
...
for cl in grid.projection.get_projected_coastlines(resolution='10m' if HIGHRES else '50m'):
    ...
    csaxes[0].plot(lon, lat, color='black', linewidth=1)
