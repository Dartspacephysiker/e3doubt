...
datapath = os.path.dirname(os.path.abspath(cs.__file__)) + '/data/natural_earth_npz/'
def get_coastlines(resolution = '50m'):
    ...
    coastlines = np.load(datapath + 'natural_earth_' + resolution + '_all.npz')
...
ax = axes[0]
plt.sca(ax)
ax.set_title("Modified Apex-110 coordinates [deg]")
...
ax.set_title(r"$\hat{\mathbf{b}}\cdot\hat{\mathbf{k}}$"+f" @ {h}-km alt [deg]")
# plt.xlabel("Geographic lon [deg]")
...
ax.clabel(imc, imc.levels, fmt=fmtzeronodeg, colors='k', inline=1, fontsize=12)
...
ax = axes[3]
plt.sca(ax)
ax.set_title(f"Main field inclination @ {h}-km alt [deg]")
plt.xlabel("Geographic lon [deg]")
